...
//	private String fileNameHero="War3humanhero.json";
//	private String fileNameUnit="war3humanunit.json";
//	private String fileNameArchi="war3humanarchitect.json";
	private String fileListName="war3listname.json";
	...
	private static String[][] fileName=new String[][]{
		{"json/war3humanhero.json","war3humanunit.json","war3humanarchitect.json"},
		{"war3nehero.json","war3neunit.json","war3nearchitect.json"},
		{"war3neutralhero.json","war3neutralunit.json","war3neutralarchitect.json"},
		{"war3orchero.json","war3orcunit.json","war3orcarchitect.json"},
		{"war3udhero.json","war3udunit.json","war3udarchitect.json"}
	};
	//set Fun ELVFragemnt，and put the int RAce into bundle,then it can be get at the onCreate()
	public ELVFragment(){}
	public static ELVFragment newInstance(int race){
		ELVFragment hf=new ELVFragment();
		Bundle bundle=new Bundle();
		bundle.putInt("race", race);		
		hf.setArguments(bundle);
		return hf;
	}
	//在 onCreate中取得，MainActivity传入的RACE值
	@Override
	public void onCreate(Bundle savedInstanceState){
		super.onCreate(savedInstanceState);
	    Bundle args=getArguments();
	    if(args!=null){
			Race=args.getInt("race");
		}
	}	
	//重写onActivityCreated()
	@Override
	...
	public void setExpandableListView(){
		...
		elv.setOnChildClickListener(new OnChildClickListener(){
		    ...
		    public boolean onChildClick(ExpandableListView parent, View v,
				...
				Toast.makeText(
						getActivity(),
	                    "你点击了"+groupPosition+"_"+childPosition,
	...
	public void getListImage(){
			imageName[0]=getName(listImageName,fileName[Race][0]);
			imageName[1]=getName(listImageName,fileName[Race][1]);
			imageName[2]=getName(listImageName,fileName[Race][2]);
			
			for(int i=0;i<imageName.length;i++){
				image[i]=new Bitmap[imageName[i].length];
				for(int j=0;j<imageName[i].length;j++){
					image[i][j]=getImage(imageName[i][j]);
				}
			}
	}		
	//得到grouparray的名称
	...
	public void getChildArrayName(){
		childArray[0]=getName(listName,fileName[Race][0]);
		childArray[1]=getName(listName,fileName[Race][1]);
		childArray[2]=getName(listName,fileName[Race][2]);
	}
	//显示详细数据
